...
namespace FortunesFromTheScrapyard.Items
{
    public class LethalInjection : FFTSItem
    {
        ...
        {
            var bar = orig(self);
            if (!self.body.bodyFlags.HasFlag(CharacterBody.BodyFlags.ImmuneToExecutes) && self.body.HasBuff(FFTSContent.Buffs.bdLethalInjection) && self.TryGetComponent<InjectionComponent>(out var component))
            {
        ...
        public class LethalInjectionBehaviour : BaseItemBodyBehavior, IOnDamageDealtServerReceiver
        {
            ...
            public void OnDamageDealtServer(DamageReport damageReport)
            {
                ...
                if (victimBody && attackerBody && damageInfo.damage / attackerBody.damage >= 4 && damageInfo.procCoefficient > 0)
                {
                    ...
                    {
                        if (NetworkServer.active) victimBody.AddBuff(FFTSContent.Buffs.bdLethalInjection);
                ...
                if (victimBody && victimHealth && victimBody.HasBuff(FFTSContent.Buffs.bdLethalInjection))
                {
