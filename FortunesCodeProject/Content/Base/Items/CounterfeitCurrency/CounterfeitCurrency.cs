...
{
    public class CounterfeitCurrency : FFTSItem
    {
        ...
        private void PurchaseInteraction_OnInteractionBegin(On.RoR2.PurchaseInteraction.orig_OnInteractionBegin orig, PurchaseInteraction self, Interactor activator)
        {
            ...
            if (self.cost > activatorBody.master.money)
            {
                ...
                int counterfeitCount = inv.GetItemCount(FFTSContent.Items.CounterfeitCurrency);
                if (counterfeitCount > 0 && activatorBody.HasBuff(FFTSContent.Buffs.bdCounterfeitLimit))
                {
        ...
        private bool PurchaseInteraction_CanBeAffordedByInteractor(On.RoR2.PurchaseInteraction.orig_CanBeAffordedByInteractor orig, PurchaseInteraction self, Interactor activator)
        {
            ...
            {
                if (activatorBody.HasBuff(FFTSContent.Buffs.bdCounterfeitLimit))
                {
        ...
        {
            return FFTSAssets.LoadAssetAsync<ItemAssetCollection>("acCounterfeitCurrency", FFTSBundle.Items);
        }
        ...
        public void CounterfeitCalculations(PurchaseInteraction self, HealthComponent activatorHealthComponent, float value)
        {
            ...
            {
                activatorHealthComponent.body.RemoveBuff(FFTSContent.Buffs.bdCounterfeitLimit);
            }
        }
        ...
        {
            [ItemDefAssociation]
            public static ItemDef GetItemDef() => FFTSContent.Items.CounterfeitCurrency;

            private void OnEnable()
            {
                if (NetworkServer.active)
                {
                    body.SetBuffCount(FFTSContent.Buffs.bdCounterfeitLimit.buffIndex, (int)GetStackValue(maxChests, maxChestsStack, body.GetItemCount(GetItemDef())));
                }
            }
            ...
            private void OnDisable()
            {
                if (NetworkServer.active)
                {
                    body.SetBuffCount(FFTSContent.Buffs.bdCounterfeitLimit.buffIndex, 0);
                }
            }
        }
    }
