    ...
    public class Takeout : FFTSItem
    {
        ...
        public class TakeoutBehaviour : BaseItemBodyBehavior, IBodyStatArgModifier
        {
                ...
                set
                {
                    if (indicatorEnabled != value)
                    {
                        if (value)
                        {
                            ...
                            if (body.HasBuff(FFTSContent.Buffs.bdTakeoutDmg)) original = chickenRadiusEffect;
                            else if (body.HasBuff(FFTSContent.Buffs.bdTakeoutSpeed)) original = noodlesRadiusEffect;
                            else if (body.HasBuff(FFTSContent.Buffs.bdTakeoutRegen)) original = potstickersRadiusEffect;
            ...
            private void OnDisable()
            {
                if (NetworkServer.active)
                {
                    if (body.HasBuff(FFTSContent.Buffs.bdTakeoutDmg))
                    {
                        if (body.HasBuff(FFTSContent.Buffs.bdChicken)) body.RemoveBuff(FFTSContent.Buffs.bdChicken);
                        if (body.HasBuff(FFTSContent.Buffs.bdChickenCooldown)) body.RemoveBuff(FFTSContent.Buffs.bdChickenCooldown);
                        body.RemoveBuff(FFTSContent.Buffs.bdTakeoutDmg);
                    }
                    else if (body.HasBuff(FFTSContent.Buffs.bdTakeoutSpeed))
                    {
                        body.RemoveBuff(FFTSContent.Buffs.bdTakeoutSpeed);
                    }
                    else if (body.HasBuff(FFTSContent.Buffs.bdTakeoutRegen))
                    {
                        body.RemoveBuff(FFTSContent.Buffs.bdTakeoutRegen);
                    }
                }
