...
namespace FortunesFromTheScrapyard.Survivors.Cloaker
{
    public class Cloaker : FFTSSurvivor
    {
        ...
        public override void Initialize()
        {
            ...
            bool tempAdd(CharacterBody body) => body.HasBuff(FFTSContent.Buffs.bdCloakerMarked);
        ...
        {
            return FFTSAssets.LoadAssetAsync<SurvivorAssetCollection>("acCloaker", FFTSBundle.Survivors);
        }
        private void CreateEffects()
        {
            cloakerConsumeEffect = assetCollection.FindAsset<GameObject>("CloakerMarkedConsumeEffect");

            FFTSContent.CreateAndAddEffectDef(cloakerConsumeEffect);
        ...
        private void GlobalEventManager_onServerDamageDealt(DamageReport damageReport)
        {
            ...
            {
                if (damageReport.damageInfo.HasModdedDamageType(CloakerAkimboDamageType) && !victimBody.HasBuff(FFTSContent.Buffs.bdCloakerMarkCd))
                {
        ...
        private void HealthComponent_TakeDamageProcess(On.RoR2.HealthComponent.orig_TakeDamageProcess orig, HealthComponent self, DamageInfo damageInfo)
        {
            ...
            if (damageInfo.attacker)
            {
                ...
                {
                    if (victimBody.HasBuff(FFTSContent.Buffs.bdCloakerMarked))
                    {
                        if (NetworkServer.active)
                        {
                            victimBody.RemoveBuff(FFTSContent.Buffs.bdCloakerMarked);
                            victimBody.AddTimedBuff(FFTSContent.Buffs.bdCloakerMarkCd, 5f);
                        }
